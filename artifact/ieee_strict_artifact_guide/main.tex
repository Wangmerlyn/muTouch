...
\title{Artifact: {$\mu$}Touch: Enabling Accurate, Lightweight Self-Touch Sensing with Passive Magnets}
...
\begin{enumerate}
  ...
  \begin{verbatim}
REPO_SSH=git@github.com:Wangmerlyn/muTouch.git
git clone --recurse-submodules "$REPO_SSH"
# HTTPS fallback:
# REPO_HTTPS=https://github.com/Wangmerlyn/muTouch.git
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item \path{read_raw_ble/read_sensor.py}
    \item \path{read_raw_ble/read_sensor_real.py}
    \item \path{read_raw_ble/read_sensor_real_classifier.py} (if used)
  \end{itemize}
...
python read_raw_ble/read_sensor.py
  ...
  \item Models: ensure \path{read_raw_ble/models/} holds the downloaded checkpoint set if you need pretrained classifiers.
\end{enumerate}
...
\begin{verbatim}
SCRIPT=read_raw_ble/read_sensor_real.py
python "$SCRIPT"
\end{verbatim}
...
\begin{verbatim}
SCRIPT=read_raw_ble/read_sensor_real_classifier.py
python "$SCRIPT"
\end{verbatim}
Ensure the script uses the latest \texttt{offset-*}, \texttt{scale-*}, and model files (picked automatically if they sit in \path{calibration_files/} and \path{read_raw_ble/models/}).
