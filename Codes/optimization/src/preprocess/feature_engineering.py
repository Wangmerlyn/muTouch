...
def feature_eng(X, pSensor):
    ...
    for i in range(pSensor.shape[0]):
        for j in range(i + 1, pSensor.shape[0]):
            ...
            diff = np.linalg.norm(tmp, ord=2, axis=1).reshape(-1, 1)  # nsample x 1
            dis = np.linalg.norm(pSensor[i] - pSensor[j], ord=2)
    ...
    feature10 = np.mean(X.reshape(-1, pSensor.shape[0], pSensor.shape[1]), axis=1)
    ...
    feature15 = (
        X.reshape(-1, pSensor.shape[0], pSensor.shape[1]) / sensor_dis
    ).reshape(nsample, -1)
    ...
    XX = np.concatenate(
        [
            ...
            feature4,
            feature5,
            feature6.reshape(-1, 1),
            feature7,
            feature8.reshape(-1, 1),
            feature9,
            feature10,
            feature11,
            feature12,
