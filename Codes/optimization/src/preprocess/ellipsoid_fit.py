...
def ellipsoid_plot(
    center, radii, rotation, ax, plot_axes=False, cage_color="b", cage_alpha=0.2
):
    """Plot an ellipsoid"""
...
def sphere_fit(points):
    ...
    A = np.array(
        [
            [x2m - xm * xm, xym - xm * ym, xzm - xm * zm],
            [xym - xm * ym, y2m - ym * ym, yzm - ym * zm],
            [xzm - xm * zm, yzm - ym * zm, z2m - zm * zm],
        ]
    )
    b = 0.5 * np.array(
        [
            x3m - xm * x2m + y2xm - xm * y2m + z2xm - xm * z2m,
            x2ym - x2m * ym + y3m - ym * y2m + z2ym - ym * z2m,
