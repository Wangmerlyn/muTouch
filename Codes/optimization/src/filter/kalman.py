...
class My_UKF(UKF):
    ...
    def cal_Z(self, z, R=None, UT=None, hx=None, **hx_args):
        ...
        zp, S = UT(sigmas_h, self.Wm, self.Wc, R, self.z_mean, self.residual_z)
        SI = self.inv(S)
...
class Magnet_UKF:
    ...
    def __setup_UFK(self):
        points = MerweScaledSigmaPoints(
            3 + (5 * 2) * self.mag_count,
            alpha=1e-3,
            beta=2.0,
            kappa=3 - (3 + (5 * 2) * self.mag_count),
        )
        self.ukf = My_UKF(
            3 + (5 * 2) * self.mag_count,
            len(self.R_std),
        ...
        for i in range(self.mag_count):
            # x y z
            self.ukf.Q[3 + 5 * i : 3 + 5 * i + 2, 3 + 5 * i : 3 + 5 * i + 2] = (
                Q_discrete_white_noise(2, dt=self.dt, var=5e-2**2)
            )
            self.ukf.Q[3 + 5 * i + 2 : 3 + 5 * i + 4, 3 + 5 * i + 2 : 3 + 5 * i + 4] = (
                Q_discrete_white_noise(2, dt=self.dt, var=5e-2**2)
            )
            self.ukf.Q[3 + 5 * i + 4 : 3 + 5 * i + 6, 3 + 5 * i + 4 : 3 + 5 * i + 6] = (
                Q_discrete_white_noise(2, dt=self.dt, var=5e-2**2)
            )
            # theta phy
            self.ukf.Q[3 + 5 * i + 6 : 3 + 5 * i + 8, 3 + 5 * i + 6 : 3 + 5 * i + 8] = (
                Q_discrete_white_noise(2, dt=self.dt, var=5e-2)
            )
            self.ukf.Q[
                3 + 5 * i + 8 : 3 + 5 * i + 10, 3 + 5 * i + 8 : 3 + 5 * i + 10
            ] = Q_discrete_white_noise(2, dt=self.dt, var=5e-2)
    ...
    def __setup_UF(self):
        tracker = KalmanFilter(
            dim_x=3 + (5 * 2) * self.mag_count, dim_z=3 + (5 * 1) * self.mag_count
        )
        ...
        # H matrix
        H = np.zeros(
            [3 + 5 * 1 * self.mag_count, 3 + 5 * 2 * self.mag_count], dtype=float
        )
        H[0, 0] = 1
        H[1, 1] = 1
        H[2, 2] = 1
    ...
    def mag_Hx(self, x):
        ...
        for i in range(self.pSensor.shape[0]):
            params = [np.log(1.85)] + self.pSensor[i].tolist() + x[mask].tolist()
    ...
    def update(self, z):
        ...
        for i in range(self.mag_count):
            ...
            result.add(
                "theta{}".format(i), self.ukf.x[3 + 3 * self.ord + i * 5 * self.ord]
            )
            result.add(
                "phy{}".format(i), self.ukf.x[3 + 4 * self.ord + i * 5 * self.ord]
            )
        return result

        est_B = self.cal_z(z).reshape(-1, 3)

        result = self.lm_model.solve(
            est_B, self.pSensor, not self.lm_model.fit_params["m0"].vary
        )
...
class Magnet_KF:
    ...
    def __setup_KF(self):
        tracker = KalmanFilter(
            dim_x=3 + (5 * self.ord) * self.mag_count,
        ...
        # H matrix
        H = np.zeros(
            [3 + 5 * 1 * self.mag_count, 3 + 5 * self.ord * self.mag_count], dtype=float
        )
        H[0, 0] = 1
        H[1, 1] = 1
        H[2, 2] = 1
        ...
        for i in range(self.mag_count):
            tracker.Q[
                3 + 5 * self.ord * i : 3 + 5 * self.ord * i + self.ord,
                3 + 5 * self.ord * i : 3 + 5 * self.ord * i + self.ord,
            ] = Q_discrete_white_noise(self.ord, dt=self.dt, var=1e-1**2)
            tracker.Q[
                3 + 5 * self.ord * i + self.ord : 3 + 5 * self.ord * i + 2 * self.ord,
                3 + 5 * self.ord * i + self.ord : 3 + 5 * self.ord * i + 2 * self.ord,
            ] = Q_discrete_white_noise(self.ord, dt=self.dt, var=1e-1**2)
            tracker.Q[
                3 + 5 * self.ord * i + 2 * self.ord : 3
                + 5 * self.ord * i
                + 3 * self.ord,
                3 + 5 * self.ord * i + 2 * self.ord : 3
                + 5 * self.ord * i
            ...
            # theta phy
            tracker.Q[
                3 + 5 * self.ord * i + 3 * self.ord : 3
                + 5 * self.ord * i
                + 4 * self.ord,
                3 + 5 * self.ord * i + 3 * self.ord : 3
                + 5 * self.ord * i
                + 4 * self.ord,
            ] = Q_discrete_white_noise(self.ord, dt=self.dt, var=5e-2)
            tracker.Q[
                3 + 5 * self.ord * i + 4 * self.ord : 3
                + 5 * self.ord * i
                + 5 * self.ord,
                3 + 5 * self.ord * i + 4 * self.ord : 3
                + 5 * self.ord * i
        ...
        if self.ord == 3:
            tracker.x = np.array(
                [0, 0, 0]
                + [0e-2, 0, 0, 5e-2, 0, 0, 5e-2, 0, 0, 0.2, 0, 0.0, 0.0, 0, 0.0]
                * self.mag_count
            )
    ...
    def __build_exp(self):
        ...
        VecB = 1e6 * (3 * vecR * (VecM.T * vecR) / dis**5 - VecM / dis**3) + G
        # VecB = 1e6 * VecB
        self.lam_VecB = sp.lambdify(
            [M, xs, ys, zs, gx, gy, gz, x, y, z, theta, phy], VecB, "numpy"
        )
    ...
    def update(self, z):
        ...
        result = self.lm_model.solve(
            z, self.pSensor, not self.lm_model.fit_params["m0"].vary
        )

        zz = [result["gx"].value, result["gy"].value, result["gz"].value]
        for i in range(self.mag_count):
            zz += [
                result["X{}".format(i)].value,
                result["Y{}".format(i)].value,
                result["Z{}".format(i)].value,
                result["theta{}".format(i)].value,
                result["phy{}".format(i)].value,
            ]
        # print(zz[3:6])
        ...
        for i in range(self.mag_count):
            ...
            result["Z{}".format(i)].value = self.kf.x[
                3 + 2 * self.ord + i * 5 * self.ord
            ]
            result["theta{}".format(i)].value = self.kf.x[
                3 + 3 * self.ord + i * 5 * self.ord
            ]
            result["phy{}".format(i)].value = self.kf.x[
                3 + 4 * self.ord + i * 5 * self.ord
            ]
...
class Magnet_KF_cpp:
    ...
    def __setup_KF(self):
        ...
        for i in range(self.mag_count):
            tracker.Q[
                3 + 5 * self.ord * i : 3 + 5 * self.ord * i + self.ord,
                3 + 5 * self.ord * i : 3 + 5 * self.ord * i + self.ord,
            ] = Q_discrete_white_noise(self.ord, dt=self.dt, var=1e-2**2)
            tracker.Q[
                3 + 5 * self.ord * i + self.ord : 3 + 5 * self.ord * i + 2 * self.ord,
                3 + 5 * self.ord * i + self.ord : 3 + 5 * self.ord * i + 2 * self.ord,
            ] = Q_discrete_white_noise(self.ord, dt=self.dt, var=1e-2**2)
    ...
    def update(self, z):
        ...
        if self.mag_count == 1:
            result = cs.solve_1mag(z.reshape(-1), self.pSensor.reshape(-1), self.params)

        elif self.mag_count == 2:
            result = cs.solve_2mag(z.reshape(-1), self.pSensor.reshape(-1), self.params)
